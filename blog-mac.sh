...
appendHeader() {
    gsed -n 1,/$bp/p $1 | gsed \$d >> $2
}

appendContent() {
    gsed -n /$bp/,/$ep/p $1 | gsed 1d | gsed \$d >> $2
}

appendFooter() {
    gsed -n /$ep/,\$p $1 | gsed 1d >> $2
}

rm -r $dst_dir
mkdir $dst_dir

gcsplit $src_dir/page.layout "/<!-- entry -->/" {*}
...
for ((i=0;i<l;i++));do
    ...
    gsed -i "s/\$fout/$fout/" $dst_dir/sitemap.xml
    
    # index
#    appendContent $src_dir/index.layout $dst_dir/index.html
#    gsed -i "s/\$fout/$fout/" $dst_dir/index.html
#    gsed -i "s/\$day/$day/" $dst_dir/index.html
#    gsed -i "s/\$month/$month/" $dst_dir/index.html
#    gsed -i "s/\$year/$year/" $dst_dir/index.html
#    gsed -i "s/\$title/$title/" $dst_dir/index.html
    ...
    gsed -i "s/<!-- day -->/$day/" $out
    gsed -i "s/<!-- month -->/${long_months[$month_index]}/" $out
    gsed -i "s/<!-- year -->/$year/" $out
    gsed -i "s/<!-- title -->/$title/" $out
    gsed -i "s/<!-- description -->/$description/" $out
    gsed -i "/<!-- content -->/r $out.filtered" $out
    rm $out.filtered
...
for ((i=l-1;i>=0;i--));do
    ...
    gsed -i "s/\$title/$title/" $dst_dir/rss.xml
    gsed -i "s/\$fout/$fout/" $dst_dir/rss.xml
    pubDate=`gdate -R -d $year$month$day`
    gsed -i "s/\$date/$pubDate/" $dst_dir/rss.xml
    ...
    echo `expr $month - 1`
    echo $month_name
    gsed -i "s/\$month/$month_name/" $dst_dir/index.html
    gsed -i "s/\$year/$year/" $dst_dir/index.html
    gsed -i "s/\$title/$title/" $dst_dir/index.html
...
gsed -i "s/<!-- title -->/Liste des billets/" $dst_dir/index.html
